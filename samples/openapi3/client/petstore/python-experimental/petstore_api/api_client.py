...
class ApiClient(object):
    ...
    def sanitize_for_serialization(cls, obj):
        ...
        """
        if isinstance(obj, (ModelComposed)):
            return {
    ...
    def deserialize(self, response, response_cls, _check_type):
        """Deserializes response into an object.

        :param response: RESTResponse object to be deserialized.
        :param response_cls: tuple the response Schema class that we are deserializing to
        ...
        """
        # TODO remove this when I switch to passing in a class
        response_cls = response_cls[0]

        # handle file downloading
        # save response body into a tmp file and return the instance
        if response_cls == file_type:
        ...
        deserialized_data = response_cls(received_data, _from_server=True, _configuration=self.configuration)
...
class Endpoint(object):
    ...
    def __validate_inputs(self, kwargs):
        ...
        for key, value in kwargs.items():
            # TODO update this when we pass in one schema only
            item_cls = self.openapi_types[key][0]
            if not isinstance(value, item_cls):
                kwargs[key] = item_cls(value, _configuration=self.api_client.configuration, _from_server=True)
