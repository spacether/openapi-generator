...
class TestDateWithValidations(unittest.TestCase):
    ...
    def testDateWithValidations(self):
        ...
        with self.assertRaisesRegex(
            ...
            r"Invalid type. Required type is date and passed type was str at \('args\[0\]',\)"
        ):
        ...
        for valid_value in valid_values:
            inst = DateWithValidations(valid_value, _from_server=True)
            assert inst == expected_date

        # value error is raised if an invalid string is passed in
        # TODO add location data to exception
        with self.assertRaisesRegex(
            ...
            DateWithValidations("2020-01-01T00:00:00Z", _from_server=True)
        ...
        with self.assertRaisesRegex(
            ...
            DateWithValidations("abcd", _from_server=True)
        ...
        with self.assertRaisesRegex(
            ...
            DateWithValidations("2019-01-01", _from_server=True)
        with self.assertRaisesRegex(
            petstore_api.ApiValueError,
            error_regex
        ):
